...
void sp_example_write_to_gps()
{
    ...
    // Stores the nmea response from the SP145
    uint8_t nmea[1024];
    int nmealen;

    // Send get message
    spWriteToGPS(handle, msgBin.data(), msgBin.size());
    ...
    while(iter++ < 25) {
        ...
        SpBool updated;
        nmealen = 1024;
        spGetGPSInfo(handle, spTrue, &updated, 0, 0, 0, 0, (char *)nmea, &nmealen);
    ...
    if(!found) {
        ...
        printf("Did not find GPS response\n");
        exit(-1);
    }
